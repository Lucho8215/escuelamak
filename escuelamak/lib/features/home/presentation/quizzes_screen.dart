...
class QuizModel {
  ...
  Color get difficultyColor {
    switch (dificultad?.toLowerCase()) {
      case 'easy':   case 'facil':  return Colors.green;
      case 'medium': case 'medio':  return Colors.orange;
      case 'hard':   case 'dificil': return Colors.red;
...
class _QuizzesScreenState extends State<QuizzesScreen> {
  ...
  Future<void> _loadQuizzes() async {
    ...
    try {
      ...
      setState(() { _quizzes = lista; _loading = false; });
    } catch (_) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Expanded(child: Text(quiz.titulo, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w600), maxLines: 2, overflow: TextOverflow.ellipsis)),
          ...
          Row(children: [
            ...
            const SizedBox(width: 4),
            Text('${quiz.tiempoLimite} min', style: TextStyle(fontSize: 12, color: Colors.grey.shade600)),
            const SizedBox(width: 12),
            // Puntaje mínimo
            Icon(Icons.star_rounded, size: 14, color: Colors.grey.shade500),
            const SizedBox(width: 4),
            Text('Min: ${quiz.puntajeMinimo}%', style: TextStyle(fontSize: 12, color: Colors.grey.shade600)),
            const Spacer(),
            // Dificultad
            if (quiz.dificultad != null)
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                decoration: BoxDecoration(color: quiz.difficultyColor.withOpacity(0.1), borderRadius: BorderRadius.circular(6)),
                child: Text(quiz.difficultyLabel, style: TextStyle(fontSize: 10, color: quiz.difficultyColor, fontWeight: FontWeight.w600)),
              ),
          ]),
          if (quiz.dueDate != null) ...[
            const SizedBox(height: 8),
            Row(children: [
              Icon(Icons.calendar_today_rounded, size: 13, color: quiz.venceHoy ? Colors.orange : Colors.grey.shade500),
              const SizedBox(width: 4),
              Text(
                'Vence: ${DateFormat('dd MMM yyyy', 'es').format(quiz.dueDate!)}',
                style: TextStyle(fontSize: 12, color: quiz.venceHoy ? Colors.orange : Colors.grey.shade600, fontWeight: quiz.venceHoy ? FontWeight.w600 : FontWeight.normal),
              ),
            ]),
          ],
        ]),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          // Título
          FadeSlideIn(child: Text(quiz.titulo, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w700))),
          ...
          if (quiz.descripcion != null)
            FadeSlideIn(
              ...
              child: Container(
                ...
                child: Text(quiz.descripcion!, style: const TextStyle(fontSize: 14, height: 1.6)),
              ),
            ),
          ...
          FadeSlideIn(
            ...
            child: Row(children: [
              Expanded(child: _InfoCard(Icons.timer_rounded, '${quiz.tiempoLimite} min', 'Tiempo límite', Colors.blue)),
              const SizedBox(width: 12),
              Expanded(child: _InfoCard(Icons.star_rounded, '${quiz.puntajeMinimo}%', 'Para aprobar', Colors.green)),
              const SizedBox(width: 12),
              Expanded(child: _InfoCard(Icons.speed_rounded, quiz.difficultyLabel, 'Dificultad', quiz.difficultyColor)),
            ]),
          ),
          ...
          if (quiz.isCompleted) ...[
            FadeSlideIn(
              ...
              child: Container(
                ...
                padding: const EdgeInsets.all(20),
                decoration: BoxDecoration(
                  color: (quiz.passed == true ? Colors.green : Colors.red).withOpacity(0.08),
                  borderRadius: BorderRadius.circular(16),
                  border: Border.all(color: (quiz.passed == true ? Colors.green : Colors.red).withOpacity(0.3)),
                ),
                child: Column(children: [
                  Icon(quiz.passed == true ? Icons.emoji_events_rounded : Icons.sentiment_dissatisfied_rounded,
                    size: 48, color: quiz.passed == true ? Colors.green : Colors.red),
                  const SizedBox(height: 8),
                  Text(quiz.passed == true ? '¡Aprobado!' : 'No aprobado',
                    style: TextStyle(fontSize: 20, fontWeight: FontWeight.w700, color: quiz.passed == true ? Colors.green : Colors.red)),
          ...
          FadeSlideIn(
            ...
            child: ElevatedButton.icon(
              icon: Icon(quiz.isCompleted ? Icons.replay_rounded : Icons.play_arrow_rounded),
              label: Text(quiz.isCompleted ? 'Intentar de nuevo' : 'Comenzar Quiz'),
...
class _QuizPlayScreenState extends State<QuizPlayScreen> {
  ...
  Future<void> _loadPreguntas() async {
    try {
      ...
      setState(() {
        _preguntas = (res as List).map((j) => QuestionModel.fromJson(j)).toList();
        _loading   = false;
      });
  ...
  Future<void> _enviarQuiz({bool tiempoAgotado = false}) async {
    ...
    try {
      ...
      if (widget.quiz.assignmentId != null) {
        await _supabase.from('quiz_assignments')
  ...
  Widget build(BuildContext context) {
    ...
    if (_preguntas.isEmpty) return Scaffold(
      appBar: AppBar(title: const Text('Quiz'), backgroundColor: Colors.purple.shade700, foregroundColor: Colors.white),
    ...
    return WillPopScope(
      onWillPop: () async {
        final salir = await showDialog<bool>(context: context, builder: (ctx) => AlertDialog(
          title: const Text('Salir del quiz'),
          content: const Text('Si sales perderás tu progreso. ¿Estás seguro?'),
          actions: [TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('Continuar')), ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: Colors.red, foregroundColor: Colors.white), onPressed: () => Navigator.pop(ctx, true), child: const Text('Salir'))],
        ));
        return salir ?? false;
      },
      child: Scaffold(
        appBar: AppBar(
          ...
          actions: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color: _tiempoCritico ? Colors.red.withOpacity(0.3) : Colors.white.withOpacity(0.2),
                borderRadius: BorderRadius.circular(20),
              ),
              child: Row(children: [
                Icon(Icons.timer_rounded, size: 16, color: _tiempoCritico ? Colors.red.shade200 : Colors.white),
                const SizedBox(width: 4),
                Text(_tiempoFormateado, style: TextStyle(color: _tiempoCritico ? Colors.red.shade200 : Colors.white, fontWeight: FontWeight.w700)),
              ]),
            ),
          ],
        ),
        ...
        body: Column(children: [
            ...
            child: SingleChildScrollView(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Container(
                  width: double.infinity,
                  padding: const EdgeInsets.all(16),
                  decoration: BoxDecoration(color: Colors.purple.shade50, borderRadius: BorderRadius.circular(14), border: Border.all(color: Colors.purple.shade100)),
                  child: Text(pregunta.texto, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600, height: 1.5)),
                ),
                const SizedBox(height: 20),
                ...
                ...pregunta.opciones.asMap().entries.map((e) {
                  ...
                  return GestureDetector(
                    ...
                    child: AnimatedContainer(
                      ...
                      padding: const EdgeInsets.all(14),
                      decoration: BoxDecoration(
                        color: selected ? Colors.purple.shade50 : Theme.of(context).cardColor,
                        borderRadius: BorderRadius.circular(12),
                        border: Border.all(color: selected ? Colors.purple.shade700 : Colors.grey.shade300, width: selected ? 2 : 1),
                      ),
                      child: Row(children: [
                        Container(
                          width: 28, height: 28,
                          decoration: BoxDecoration(
                            color: selected ? Colors.purple.shade700 : Colors.grey.shade200,
                            shape: BoxShape.circle,
                          ),
                          child: Center(child: Text(
                            String.fromCharCode(65 + i), // A, B, C, D
                            style: TextStyle(color: selected ? Colors.white : Colors.grey.shade600, fontWeight: FontWeight.w700, fontSize: 13),
                          )),
                        ),
                        const SizedBox(width: 12),
                        Expanded(child: Text(opcion, style: TextStyle(fontSize: 14, color: selected ? Colors.purple.shade800 : null, fontWeight: selected ? FontWeight.w600 : FontWeight.normal))),
                        if (selected) Icon(Icons.check_circle_rounded, color: Colors.purple.shade700, size: 20),
                      ]),
                    ),
                  );
                }).toList(),
          ...
          Container(
            ...
            child: Row(children: [
              if (_actual > 0)
                Expanded(child: OutlinedButton(onPressed: _anterior, style: OutlinedButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 14), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))), child: const Text('Anterior'))),
              if (_actual > 0) const SizedBox(width: 12),
              Expanded(
                flex: 2,
                child: ElevatedButton(
                  onPressed: respuesta != null ? _siguiente : null,
                  style: ElevatedButton.styleFrom(backgroundColor: Colors.purple.shade700, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(vertical: 14), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), disabledBackgroundColor: Colors.grey.shade300),
                  child: Text(_actual < _preguntas.length - 1 ? 'Siguiente' : 'Enviar Quiz', style: const TextStyle(fontWeight: FontWeight.w700)),
                ),
              ),
            ]),
          ),
      ...
      body: SingleChildScrollView(
        child: Column(children: [
          ...
          Container(
            ...
            child: Column(children: [
              Icon(passed ? Icons.emoji_events_rounded : Icons.sentiment_dissatisfied_rounded, size: 72, color: color),
              const SizedBox(height: 12),
              Text(passed ? '¡Aprobado!' : 'No aprobado', style: TextStyle(fontSize: 28, fontWeight: FontWeight.w800, color: color)),
              const SizedBox(height: 8),
              Text('$score%', style: TextStyle(fontSize: 48, fontWeight: FontWeight.w900, color: color)),
              const SizedBox(height: 4),
              Text('$correctas de ${preguntas.length} correctas', style: const TextStyle(fontSize: 16, color: Colors.grey)),
            ]),
          ),
          ...
          Padding(
            padding: const EdgeInsets.all(16),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('Revisión de respuestas', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700)),
              const SizedBox(height: 12),
              ...preguntas.asMap().entries.map((e) {
                ...
                return Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    borderRadius: BorderRadius.circular(12),
                    border: Border.all(color: correcto ? Colors.green.shade200 : Colors.red.shade200),
                  ),
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Row(children: [
                      Icon(correcto ? Icons.check_circle_rounded : Icons.cancel_rounded, color: correcto ? Colors.green : Colors.red, size: 18),
                      const SizedBox(width: 8),
                      Expanded(child: Text('${e.key + 1}. ${p.texto}', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600))),
                    ]),
                    const SizedBox(height: 8),
                    if (resp != null && resp < p.opciones.length)
                      Text('Tu respuesta: ${p.opciones[resp]}', style: TextStyle(fontSize: 12, color: correcto ? Colors.green.shade700 : Colors.red.shade700)),
                    Text('Respuesta correcta: ${p.opciones[p.respuestaCorrecta]}', style: TextStyle(fontSize: 12, color: Colors.green.shade700, fontWeight: FontWeight.w600)),
                    if (p.explicacion != null) ...[
                      const SizedBox(height: 6),
                      Text(p.explicacion!, style: TextStyle(fontSize: 12, color: Colors.grey.shade600, fontStyle: FontStyle.italic)),
                    ],
                  ]),
                );
              }).toList(),
