...
class QuestionModel {
  ...
  factory QuestionModel.fromJson(Map<String, dynamic> j) {
    ...
    // options puede venir como List (JSONB nativo) o como String (JSON.stringify)
    if (opts is String) {
      try {
        opts = jsonDecode(opts);
      } catch (_) {
        opts = [];
      }
    }
...
class _QuizPlayScreenState extends State<QuizPlayScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if (_error != null || _preguntas.isEmpty) {
      return Scaffold(
        appBar: AppBar(
        ...
        body: Center(
          child: Padding(
            ...
            child: Text(_error ?? 'No hay preguntas disponibles',
                textAlign: TextAlign.center,
